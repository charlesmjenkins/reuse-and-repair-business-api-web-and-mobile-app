...
// Title: api.php
//
// Description: The single file PHP_CRUD_API
// library that turns our MySQL database into
// a web service.
// ---------------------------------------
// Acknowledgement: This library was written
// by https://github.com/mevdschee/php-crud-api
...
$api = new MySQL_CRUD_API(array(
	...
	'table_authorizer'=>
	function($cmd,$db,$tab) {
       if($GLOBALS['password'] === "umf349MFM9m3m9HDH23MhKNP800m0MFqj0a2"){
           return ((($cmd=='list'||$cmd='read'||$cmd='create'||$cmd='update'||$cmd='delete') && $tab=='category') ||(($cmd=='list'||$cmd='read'||$cmd='create'||$cmd='update'||$cmd='delete') && $tab=='business')||(($cmd=='list'||$cmd='read'||$cmd='create'||$cmd='update'||$cmd='delete') && $tab=='item')||(($cmd=='list'||$cmd='read'||$cmd='create'||$cmd='update'||$cmd='delete') && $tab=='item-business')||(($cmd=='list'||$cmd='read'||$cmd='create'||$cmd='update'||$cmd='delete') && $tab=='item-category')||(($cmd=='list'||$cmd='read'||$cmd='create'||$cmd='update'||$cmd='delete') && $tab=='admin'));
       }
       else{
           return ((($cmd=='read'||$cmd=='list') && $tab=='category')||(($cmd=='read'||$cmd=='list') && $tab=='business')||(($cmd=='read'||$cmd=='list') && $tab=='item')||(($cmd=='read'||$cmd=='list') && $tab=='item-business')||(($cmd=='read'||$cmd=='list') && $tab=='item-category'));
       }
   }
));
$api->executeCommand();